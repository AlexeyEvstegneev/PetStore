...
const Product: React.FC<IProduct> = ({ ...products }) => {
	const user = useContext(UserContext);
	const like = user ? products.likes.includes(user._id) : false;

	return (
		<>
			<div className='product'>
				<div className='product__img-wrapper'>
					<img src={products.pictures} alt='' />
				</div>
				<div className='product__desc'>
					...
					<div className='product__btn-wrap'>
						<div className='button-count'>
							...
							<input type='number' className='button-count__num' value='0' />
							<button className='button-count__plus'>+</button>
						</div>
						<a className='button button_type_primary'>В корзину</a>
					</div>
					<button
					// className={
					// 	like
					// 		? 'product__favorite_active svg path:first-child '
					// 		: 'product__favorite '
					// }
					>
						<svg
